...
import { Bus, Search, ChevronDown, X } from 'lucide-react'
...
export default function UserMapPage() {
  ...
  useEffect(() => {
    supabase.from('bus_lines').select('*').eq('is_active', true).then(({ data }) => {
      // Use real data if available, otherwise fall back to mock lines
          ...
          >
            {selectedLine ? (
              <>
                ...
                <span className="font-display" style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: '14px', letterSpacing: '-0.01em' }}>
